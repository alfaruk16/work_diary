...
import 'package:work_diary/features/app/presentation/activities/visit_cancelled/bloc/visit_cancelled_bloc.dart';
import 'package:work_diary/features/app/presentation/activities/visit_cancelled/widget/reason.dart';

class VisitCancelledScreen extends StatelessWidget {
  const VisitCancelledScreen({Key? key, required this.visitData})
      : super(key: key);
  final VisitData visitData;

  static Route<dynamic> route({required VisitData visitData}) =>
      MaterialPageRoute<dynamic>(
        builder: (_) => VisitCancelledScreen(
          visitData: visitData,
        ),
      );
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          VisitCancelledBloc(getIt<IFlutterNavigator>(), getIt<ApiRepo>())
            ..add(VisiDetailsById(visitData: visitData)),
      child: const VisitCancelledView(),
    );
  }
}

class VisitCancelledView extends StatelessWidget {
  const VisitCancelledView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final bloc = context.read<VisitCancelledBloc>();
    final size = MediaQuery.of(context).size;

    return BlocBuilder<VisitCancelledBloc, VisitCancelledState>(
