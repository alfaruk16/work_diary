...
import 'package:work_diary/features/app/presentation/activities/todays_visit/bloc/today_visit_supervisor_state.dart';
...
class TodaysVisitBlocSupervisor
    extends Bloc<TodaysVisitEvent, TodaysVisitStateSuperVisor> {
  TodaysVisitBlocSupervisor(
      this._iFlutterNavigator, this._apiRepo, this._localStorageRepo)
      : super(const TodaysVisitStateSuperVisor()) {
  ...
  FutureOr<void> _isSupervisor(
      IsSupervisor event, Emitter<TodaysVisitStateSuperVisor> emit) async {
    emit(state.copyWith(
        isSupervisor:
  ...
  FutureOr<void> _goToPlanList(
      GoToPlanList event, Emitter<TodaysVisitStateSuperVisor> emit) {
  ...
  FutureOr<void> _getVisitStatus(
      GetVisitStatus event, Emitter<TodaysVisitStateSuperVisor> emit) async {
    final visitStatusData = await _localStorageRepo.readModel(
        key: visitStatusSupervisorDB, model: const VisitStatus());
    ...
    final visitStatusResponse = await _apiRepo.post(
        endpoint: getVisitStatusEndpoint, responseModel: const VisitStatus());
    ...
    if (visitStatusResponse != null) {
      ...
      _localStorageRepo.writeModel(
          key: visitStatusSupervisorDB, value: visitStatusResponse);
    }
  }

  FutureOr<void> _selectedVisitStatus(
      SelectedVisitStatus event, Emitter<TodaysVisitStateSuperVisor> emit) {
  ...
  FutureOr<void> _getTodaysVisitPlan(GetTodaysVisitPlan event,
      Emitter<TodaysVisitStateSuperVisor> emit) async {
    if (!state.isLoading) {
      emit(state.copyWith(isLoading: true, page: 1, isEndList: false));

      final String localDatabaseKey = todayVisitSupervisorDB +
          state.userType +
  ...
  FutureOr<void> _selectedVisitDate(
      SelectedVisitDate event, Emitter<TodaysVisitStateSuperVisor> emit) {
  ...
  FutureOr<void> _updateVisitDate(
      UpdateVisitDate event, Emitter<TodaysVisitStateSuperVisor> emit) {
  ...
  FutureOr<void> _pageIncrement(
      PageIncrement event, Emitter<TodaysVisitStateSuperVisor> emit) async {
    int totalPage = state.page + 1;
    if (totalPage <= state.todayVisits.meta!.lastPage!) {
      if (!state.incrementLoader) {
        emit(state.copyWith(incrementLoader: true, page: totalPage));
        ...
        emit(state.copyWith(incrementLoader: false));
  ...
  FutureOr<void> _getComments(
      GetComments event, Emitter<TodaysVisitStateSuperVisor> emit) {
    changeStatus(
  ...
  FutureOr<void> _updateStatus(
      UpdateStatus event, Emitter<TodaysVisitStateSuperVisor> emit) async {
  ...
  FutureOr<void> _updateVisitItem(
      UpdateVisitItem event, Emitter<TodaysVisitStateSuperVisor> emit) {
  ...
  FutureOr<void> _setPlanListType(
      SetPlanListType event, Emitter<TodaysVisitStateSuperVisor> emit) {
  ...
  FutureOr<void> _checkLocal(
      CheckLocal event, Emitter<TodaysVisitStateSuperVisor> emit) async {
    final String localDatabaseKey = todayVisitSupervisorDB +
        state.userType +
