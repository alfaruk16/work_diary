...
import 'package:work_diary/features/app/presentation/activities/emergency_issue/bloc/emergency_issue_state_own.dart';
...
class EmergencyIssueBlocOwn
    extends Bloc<EmergencyIssueEvent, EmergencyIssueStateOwn> {
  EmergencyIssueBlocOwn(
      this._iFlutterNavigator, this._apiRepo, this._localStorageRepo)
      : super(const EmergencyIssueStateOwn()) {
    ...
    on<UpdateVisiteDate>(_updateVisiteDate);
  ...
  FutureOr<void> _createEmergencyIssue(
      CreateEmergencyIssue event, Emitter<EmergencyIssueStateOwn> emit) {
    _iFlutterNavigator
  ...
  FutureOr<void> _goToPlanList(
      GoToPlanList event, Emitter<EmergencyIssueStateOwn> emit) {
  ...
  FutureOr<void> _getVisitStatus(
      GetVisitStatus event, Emitter<EmergencyIssueStateOwn> emit) async {
...
//Selected visit status
  FutureOr<void> _selectedVisitStatus(
      SelectedVisitStatus event, Emitter<EmergencyIssueStateOwn> emit) {
...
//Initial visit plans
  FutureOr<void> _getTodaysVisitPlan(
      GetTodaysVisitPlan event, Emitter<EmergencyIssueStateOwn> emit) async {
    if (!state.isLoading) {
      ...
      final String localDatabaseKey =
          emergencyIssueOwnDB + state.userType + state.selectedDate.toString();
  ...
  FutureOr<void> _selectedVisitDate(
      SelectedVisitDate event, Emitter<EmergencyIssueStateOwn> emit) {
  ...
  FutureOr<void> _updateVisiteDate(
      UpdateVisiteDate event, Emitter<EmergencyIssueStateOwn> emit) {
  ...
  FutureOr<void> _pageIncrement(
      PageIncrement event, Emitter<EmergencyIssueStateOwn> emit) async {
  ...
  FutureOr<void> _menuItemScreens(
      MenuItemScreens event, Emitter<EmergencyIssueStateOwn> emit) {
  ...
  FutureOr<void> _reload(Reload event, Emitter<EmergencyIssueStateOwn> emit) {
    emit(state.copyWith(page: 1));
    add(GetTodaysVisitPlan());
  }

  FutureOr<void> _getComments(
      GetComments event, Emitter<EmergencyIssueStateOwn> emit) {
    changeStatus(
      _iFlutterNavigator.context,
      getComment: (value) {
        _iFlutterNavigator.pop();
  ...
  FutureOr<void> _updateStatus(
      UpdateStatus event, Emitter<EmergencyIssueStateOwn> emit) async {
  ...
  FutureOr<void> _checkLocal(
      CheckLocal event, Emitter<EmergencyIssueStateOwn> emit) async {
    final String localDatabaseKey =
        emergencyIssueOwnDB + state.userType + state.selectedDate.toString();
