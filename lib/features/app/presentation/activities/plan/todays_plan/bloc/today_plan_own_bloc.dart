...
import 'package:work_diary/features/app/presentation/activities/plan/todays_plan/bloc/bloc.dart';
import 'package:work_diary/features/app/presentation/activities/plan/todays_plan/bloc/today_plan_own_state.dart';
...
class TodaysPlanBlocOwn extends Bloc<TodaysPlanEvent, TodaysPlanStateOwn> {
  TodaysPlanBlocOwn(
      this._iFlutterNavigator, this._apiRepo, this._localStorageRepo)
      : super(const TodaysPlanStateOwn()) {
    on<GetVisitStatus>(_getVisitStatus);
    on<GetTodaysPlan>(_getTodaysPlan);
  ...
  FutureOr<void> _isSupervisor(
      IsSupervisor event, Emitter<TodaysPlanStateOwn> emit) async {
    emit(state.copyWith(
        isSupervisor:
  ...
  FutureOr<void> _goToPlanList(
      GoToPlanList event, Emitter<TodaysPlanStateOwn> emit) {
  ...
  FutureOr<void> _getVisitStatus(
      GetVisitStatus event, Emitter<TodaysPlanStateOwn> emit) async {
  ...
  FutureOr<void> _selectedVisitStatus(
      SelectedVisitStatus event, Emitter<TodaysPlanStateOwn> emit) {
  ...
  FutureOr<void> _getTodaysPlan(
      GetTodaysPlan event, Emitter<TodaysPlanStateOwn> emit) async {
    if (!state.isLoading) {
      emit(state.copyWith(isLoading: true, page: 1, isEndList: false));

      final String localDatabaseKey =
          todayPlanOwnDB + state.userType + state.selectedDate.toString();
  ...
  FutureOr<void> _selectedVisitDate(
      SelectedVisitDate event, Emitter<TodaysPlanStateOwn> emit) {
  ...
  FutureOr<void> _updateVisitDate(
      UpdateVisitDate event, Emitter<TodaysPlanStateOwn> emit) {
  ...
  FutureOr<void> _pageIncrement(
      PageIncrement event, Emitter<TodaysPlanStateOwn> emit) async {
  ...
  FutureOr<void> _getComments(
      GetComments event, Emitter<TodaysPlanStateOwn> emit) {
    changeStatus(
  ...
  FutureOr<void> _updateStatus(
      UpdateStatus event, Emitter<TodaysPlanStateOwn> emit) async {
  ...
  FutureOr<void> _updateVisitItem(
      UpdateVisitItem event, Emitter<TodaysPlanStateOwn> emit) {
  ...
  FutureOr<void> _setPlanListType(
      SetPlanType event, Emitter<TodaysPlanStateOwn> emit) {
  ...
  FutureOr<void> _checkLocal(
      CheckLocal event, Emitter<TodaysPlanStateOwn> emit) async {
    final String localDatabaseKey =
        todayPlanOwnDB + state.userType + event.selectedDateDropdown.toString();
