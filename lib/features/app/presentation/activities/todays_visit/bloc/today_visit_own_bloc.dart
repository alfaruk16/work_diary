...
class TodaysVisitBlocOwn extends Bloc<TodaysVisitEvent, TodaysVisitStateOwn> {
  TodaysVisitBlocOwn(
      this._iFlutterNavigator, this._apiRepo, this._localStorageRepo)
      : super(const TodaysVisitStateOwn()) {
  ...
  FutureOr<void> _isSupervisor(
      IsSupervisor event, Emitter<TodaysVisitStateOwn> emit) async {
    emit(state.copyWith(
        isSupervisor:
  ...
  FutureOr<void> _goToPlanList(
      GoToPlanList event, Emitter<TodaysVisitStateOwn> emit) {
    if (event.planType == "Completed") {
      ...
      _iFlutterNavigator
        ...
        if (value != null) {
          _iFlutterNavigator
              .push(OngoingVisitPlanScreen.route(visitData: value))
  ...
  FutureOr<void> _getVisitStatus(
      GetVisitStatus event, Emitter<TodaysVisitStateOwn> emit) async {
    final visitStatusData = await _localStorageRepo.readModel(
        key: visitStatusOwnDB, model: const VisitStatus());
    ...
    if (visitStatusResponse != null) {
      ...
      _localStorageRepo.writeModel(
          key: visitStatusOwnDB, value: visitStatusResponse);
    }
  }

  FutureOr<void> _selectedVisitStatus(
      SelectedVisitStatus event, Emitter<TodaysVisitStateOwn> emit) {
  ...
  FutureOr<void> _getTodaysVisitPlan(
      GetTodaysVisitPlan event, Emitter<TodaysVisitStateOwn> emit) async {
    if (!state.isLoading) {
      emit(state.copyWith(isLoading: true, page: 1, isEndList: false));

      final String localDatabaseKey =
          todayVisitOwnDB + state.userType + state.selectedDate.toString();
  ...
  FutureOr<void> _selectedVisitDate(
      SelectedVisitDate event, Emitter<TodaysVisitStateOwn> emit) {
    ...
    if (event.selectedTab == null || event.selectedTab == 0) {
      add(GetTodaysVisitPlan());
    }
  }

  FutureOr<void> _updateVisitDate(
      UpdateVisitDate event, Emitter<TodaysVisitStateOwn> emit) {
  ...
  FutureOr<void> _pageIncrement(
      PageIncrement event, Emitter<TodaysVisitStateOwn> emit) async {
  ...
  FutureOr<void> _getComments(
      GetComments event, Emitter<TodaysVisitStateOwn> emit) {
    changeStatus(
  ...
  FutureOr<void> _updateStatus(
      UpdateStatus event, Emitter<TodaysVisitStateOwn> emit) async {
  ...
  FutureOr<void> _updateVisitItem(
      UpdateVisitItem event, Emitter<TodaysVisitStateOwn> emit) {
  ...
  FutureOr<void> _setPlanListType(
      SetPlanListType event, Emitter<TodaysVisitStateOwn> emit) {
  ...
  FutureOr<void> _checkLocal(
      CheckLocal event, Emitter<TodaysVisitStateOwn> emit) async {
    final String localDatabaseKey = todayVisitOwnDB +
        state.userType +
